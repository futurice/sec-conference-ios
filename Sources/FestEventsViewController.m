...
- (void)viewDidLoad
{
    ...
    [gigsSignal subscribeNext:^(NSArray *events) {
        ...
        if ([[self.gigs firstObject] isKindOfClass:[Event class]]) {
            self.days = @[[self.gigs filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"day contains[c] 'Friday' AND bar_camp == NO"]],
                          [self.gigs filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"day contains[c] 'Saturday' AND bar_camp == NO"]]
                          ];
        }
